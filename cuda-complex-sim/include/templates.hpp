 ...
 * Nota bene: adesso funziona, per favore di cristo non toccarla pi�.
 */

template <typename T>
__device__ inline void copyToTile(T* source, T* tile, int32_t start, uint16_t elements_per_thread, int16_t tile_offset){		//elements_per_thread indica quanti elementi deve copiare ciascun thread. Cos� ad esempio se � uguale a 5 e ogni blocco � formato da 10 thread, in totale verranno copiati nella shared memory 50 elementi
